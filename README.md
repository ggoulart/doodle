...
- **`MeetingService.bookSlot` has a check-then-act race window.** Two concurrent bookings for the same slot could both pass the "not already booked" / "is FREE" checks before either commits, since there's no unique DB constraint on `meetings.slot_id` backing the invariant.
- **No DB-level uniqueness constraints** on `calendars.user_id` or `meetings.slot_id` — both are enforced only in application logic, not by the schema.
