...
`/actuator/health` is a real check, not decoration: since the app has a Postgres `DataSource`, Boot auto-adds a DB health indicator, so the endpoint reports `DOWN` (with the actual JDBC error) if Postgres becomes unreachable, and recovers to `UP` once it's back — confirmed by stopping Postgres and watching it happen live. It's the only health endpoint the app exposes — a previous hand-written `GET /health` (always `200`, no dependencies) was removed in favor of this single dependency-aware one.
...
- **All Actuator endpoints are exposed with no authentication.** `management.endpoints.web.exposure.include: "*"` opens every Actuator endpoint, not just `/actuator/health` — including `env`, `beans`, `heapdump`, and `threaddump` — with nothing gating access. Fine for local/demo use given the Admin server and Swagger UI are already unauthenticated by the same tradeoff, but it would need locking down (e.g. restricting exposure to `health,info` publicly, securing the rest) before running anywhere reachable outside localhost.
